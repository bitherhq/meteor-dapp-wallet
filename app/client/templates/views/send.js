...
var checkOverDailyLimit = function(address, wei, template){
    ...
    if(account && account.requiredSignatures > 1 && !_.isUndefined(account.dailyLimit) && account.dailyLimit !== bitherConfig.dailyLimitDefault && Number(wei) !== 0) {
        // check whats left
...
var getDataField = function(){
    ...
    if(selectedToken && selectedToken !== 'bither') {
...
Template['views_send'].onCreated(function(){
    ...
    if(FlowRouter.getRouteName() === 'deployContract') {
        ...
        TemplateVar.set('selectedToken', 'bither');

    // Send funds
    } else {
        TemplateVar.set('selectedAction', 'send-funds');
        TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'bither');
    }
    ...
    template.autorun(function(c){
        ...
        if(!c.firstRun && TemplateVar.get('selectedToken') === 'bither') {
...
Template['views_send'].helpers({
    ...
    */
    'total': function(bither){
        ...
        if (TemplateVar.get('selectedToken') === 'bither') {
...
Template['views_send'].events({
    ...
    */
    'click .token-bither': function(e, template){
        TemplateVar.set('selectedToken', 'bither');
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'bither'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'bither'))
                return GlobalNotification.warning({
